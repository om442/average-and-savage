...
class FormatData:
    ...
    def get_avg_pos_len(self):
        ...
        for experience in self.profile.get('experiences'):
            for roles in self.profile.get('experiences')[experience]:
                if roles['start_date'] is None:
                    start_date = self.format_date(str(datetime.now())[:7])
                else:
                ...
                self.tenure_len += (end_date - start_date).days

        if self.tenure_len == 0:
            return 0
        if start_date == "" or end_date == "":
            return 0
        return (self.tenure_len / n_pos)

    def get_n_prev_tenures(self):
        if self.profile.get('experiences'):
    ...
    def get_avg_prev_tenure_len(self, get_tenure_len=False):
        ...
        if get_tenure_len:
            return tenure_len
        n_prev_tenures = self.get_n_prev_tenures()
        if n_prev_tenures > 0:
    ...
    def get_age(self):
        ...
        if self.profile.get('birth_year') is None:
            for education in self.profile.get('education'):
                ...
                degrees = self.profile.get('education')[education]['degrees']
                if "bachelors" in degrees or "Bachelors" in degrees:
                    bach_end_date = self.profile.get('education')[education]['end_date']
                    if bach_end_date is None:
                        bach_end_year = 2020
                    else:
                        try:
                            bach_end_year = datetime.strptime(bach_end_date, '%Y').year
                        except ValueError:
                            if len(bach_end_date) >= 4: 
                                bach_end_date = int(bach_end_date[:3])
    ...
    def get_gender(self):
        if self.profile.get('gender') is None:
            return 'Male'
    ...
    def get_test_data_format(self):
        ...
        # tenure_len
        test_dict['tenure_len'] = self.get_avg_prev_tenure_len(get_tenure_len=True)
...
#     def get_avg_n_pos_per_prev_tenure(self):
#         print(type(self.profile))  ##check here
#         num_experiences = len(self.profile.get('experiences'))
#         n_pos = self.get_n_pos()
